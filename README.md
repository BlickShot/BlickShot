...
-- Update dependent GUI positions when main frame moves
...
local function createRecordSlot(name, data)
	...
	playBtn.MouseButton1Click:Connect(function()
		...
		replayConnection = RunService.RenderStepped:Connect(function()
			...
			while i <= #data and data[i].time <= now do
				hrp.CFrame = data[i].cf
				i += 1
			end
			if i > #data then
...
-- Anti-tamper to protect GUI but allow dragging
local defaultSize = UDim2.new(0, 240, 0, 300)

local function antiTamper()
	-- Ensure parents intact
	if not frame.Parent or frame.Parent ~= screenGui then
		frame.Parent = screenGui
	end
	if slotGui.Parent ~= screenGui then
		slotGui.Parent = screenGui
	end
	if credit.Parent ~= screenGui then
		credit.Parent = screenGui
	end
	...
	-- Fix size only if drastically changed
	local sizeDiffX = math.abs(frame.Size.X.Offset - defaultSize.X.Offset)
	local sizeDiffY = math.abs(frame.Size.Y.Offset - defaultSize.Y.Offset)
	if sizeDiffX > 50 or sizeDiffY > 50 then
		frame.Size = defaultSize
	end

	-- Keep draggable and active enabled
	if frame.Draggable ~= true then
		frame.Draggable = true
	end
	if frame.Active ~= true then
		frame.Active = true
	end

	-- Ensure toggleBtn parent intact
	if toggleBtn.Parent ~= frame then
		toggleBtn.Parent = frame
	end
end

RunService.Heartbeat:Connect(antiTamper)
